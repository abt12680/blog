...
新生代程序员，我已经不推荐去看 C++ 了。Java、C#、go、node.js、python，在应用程序语言群雄并立的年代，C++，必将远去。
...
这一节，想试着从历史的角度，看看 C++ 的过去和未来。不一定正确，just for fun~
...
越来越多的模块加入到标准库了，但，对比 Java、go 等语言，标准库还是不够丰富。但最核心的问题，还是 C++ 期望大而全，从最开始背负了太多内容。最终导致写 C++ 的程序员，负担太重，写代码需要"脑子里背着个编译器"。
...
C++ 最大的价值，就在于还有一大堆老系统是用 C++ 写得。长期可见的未来，这一大坨 C++ 代码，还需要一大堆懂 C++ 的程序员。
 ...
 * pongba 的 [C++11（及现代C++风格）和快速迭代式开发][1]，正确使用 modern C++。从我现在的角度来看，里面谈到的很多问题，还是 C++ 语言本身带来的。
