...
### Request
Request提供了一种简化的HTTP请求方式。你可以使用比其他HTTP库更少的代码来发起HTTP请求。通过引入request-promise库，可以支持Promise请求。
...
## 选择Axios
随着Vue框架弃用vue-resource，而推荐使用axios开始，axios明显获得了更多的社区支持度。大量React项目的使用也成为了选择的主要原因之一。
...
附上git地址：[axios github](https://github.com/axios/axios)
...
```
但是如果每次都写完整的url貌似有些多余，毕竟链接的服务器在大部分情况下都是一样的，这时候可以做以下设置
```
...
### axios(config)
axios(config)使得以上请求均可以配置化，比如以上的post请求可以用该方法写成一下形式
```
axios({
    method: 'get',
...
### axios实例
之前我们为了方便配置了axios.default.baseURL，但是如果现在有多个服务器需要连接，然后就懵逼了。之下我们引入axios实例，面对不同服务器可以用多个axios实例来一一对应
`axios.create([config])`
```
