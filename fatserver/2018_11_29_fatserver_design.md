...
一直想自己写个 game server 来玩玩。以前 [fatfatson][2] 的客户端引擎叫 fatengine，我的这个玩具就叫 [fatserver][3] 好了。
...
fatfatson 给了一个用 C# 实现的分布式 game server -- [ET][1]。瞄了一眼，基本上实现了我想做的所有功能。不过代码细节上可以再扣细一点，性能应该可以更好。

想起 [Orleans][4] 和 [Akka.NET][5] 项目。认真看了下 [Akka.NET][5] 的文档。在模拟 erlang 的这条道路上，[Akka.NET][5] 已经做得很好了。从设计上来说，erlang 在通用化上做得更高，并不局限于游戏项目。
