...
好了，接下来将讲解全世界最快的 awk 学习方法。你会写C吧？你会写C吧？你会写C吧？重要的事情说三遍。=_=!
...
awk 的本质，就是三段C函数。BEGIN、END 这两个函数是可选的，分别在本次 awk 流程的开始、结束各执行一次。而中间函数，是针对每一行数据，执行一次。你就把 C 中学会的东西，往里面用，就是了。为啥会如此之像呢？因为 C 和 awk 是同一批人设计出来的嘛。哈哈~
